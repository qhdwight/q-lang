...
//    std::vector<std::string>
    ...
    void Parser::recurseNodes(std::string const& code, std::weak_ptr<AbstractNode> const& parent, int depth) {
        using nodeType = std::function<std::shared_ptr<AbstractNode>(std::string&&, AbstractNode::ParentRef)>;
        std::map<std::string, nodeType> nameToNode;
        nameToNode.emplace("package", [](auto name, auto parent) { return std::make_shared<PackageNode>(std::move(name), parent); });
        nameToNode.emplace("default", [](auto name, auto parent) { return std::make_shared<ParseNode>(std::move(name), parent); });
        auto level = 0;
...
//                    auto child = std::make_shared<ParseNode>(std::move(blockWithInfo), parent);
                    auto child = nameToNode["default"](std::move(blockWithInfo), parent);
